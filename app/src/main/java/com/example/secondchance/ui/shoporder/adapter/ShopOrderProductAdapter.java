...
public class ShopOrderProductAdapter extends RecyclerView.Adapter<ShopOrderProductAdapter.ViewHolder> {
    ...
    public ShopOrderProductAdapter(List<ShopOrderProduct> productList, OnSelectionChangedListener listener) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.item_shop_order_refund_request_detail, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ShopOrderProduct p = productList.get(position);
    ...
    private int getSelectedCount() {
        int count = 0;
        for (ShopOrderProduct p : productList) {
            if (p.isSelected()) count++;
        }
        return count;
    }

    public List<ShopOrderProduct> getSelectedProducts() {
        List<ShopOrderProduct> selected = new ArrayList<>();
        for (ShopOrderProduct p : productList) {
